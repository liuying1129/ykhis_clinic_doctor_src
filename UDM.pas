...
var
  ...
  HisConn:string;//His连接字符串,MakeDBConn过程中被赋值
...
function ShowPatientForm(AHandle:THandle;AHisConn:Pchar;AOperator:Pchar;AOperatorDep:Pchar):PChar;stdcall;external 'SelPatient.dll';
function InsertTreatMaster(AHisConn:Pchar;APatient_Unid:integer;AOperator:PChar;ADepartment:PChar;ARegister_Src:PChar;ARegister_Treat_Date:TDateTime;ARegister_Morning_Afternoon:PChar;ARegister_No_Type:PChar;ARegister_Operator:PChar):integer;stdcall;external 'SelPatient.dll';
  
function MakeDBConn:boolean;
function ExecSQLCmd(AConnectionString:string;ASQL:string):boolean;
...
function GetBracketCode(const AString:string):string;//AString:[001]张三,返回值001
function GetBracketName(const AString:string):string;//AString:[001]张三,返回值张三
function Get3SpaceCode(const AString:string):string;
function Get3SpaceName(const AString:string):string;
...
begin
  ...
  Conn.ConnectString:=AConnectionString;
...
function GetBracketCode(const AString:string):string;
//AString:[001]张三,返回值001
var
  i:integer;
begin
  if copy(AString,1,1)<>'[' then begin result:='';exit;end;
  
  i:=pos(']',AString);
  result:=copy(AString,2,i-2);
...
function GetBracketName(const AString:string):string;
//AString:[001]张三,返回值张三
var
  i,j:integer;
begin
  i:=pos(AString,'[');
  ...
  if i>=j then begin result:='';exit;end;

  result:=copy(AString,j+1,MaxInt);
end;

function Get3SpaceCode(const AString:string):string;
//AString:001   张三,返回值001.代码、名称用3个空格分隔
var
  i:integer;
begin
  i:=pos('   ',AString);
...
function Get3SpaceName(const AString:string):string;
//AString:001   张三,返回值张三.代码、名称用3个空格分隔
var
  i:integer;
begin
  i:=pos('   ',AString);
  if i<=0 then result:=AString else result:=copy(AString,i+3,MaxInt);
end;
